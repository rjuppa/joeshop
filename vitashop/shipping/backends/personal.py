...
class PersonalPickupForm(forms.Form):
...
class PersonalShipping(object):
    ...
    """
    url_namespace = 'personal'
    backend_name = 'Personal pickup'
    backend_verbose_name = _('Personal pickup')
    template = 'vitashop/shipping/personal/locations.html'

    def __init__(self, shop):
        self.shop = shop
        self.rate = 0   # it is free
    ...
    def view_process_order(self, request):
        ...
        process.
        """
        self.shop.add_shipping_costs(self.shop.get_order(request), 'Personal pickup', Decimal(self.rate))
    ...
    def view_display_fees(self, request):
        ...
        form = PersonalPickupForm()     # Czech rep.
        d = Decimal(self.rate)
        if request.method == 'POST':
            if u'locations' in request.POST:
                # Czech rep.
                form = PersonalPickupForm(data=request.POST)
    ...
    def get_urls(self):
        ...
        urlpatterns = patterns('',
            url(r'^$', self.view_display_fees, name='personal'),
            url(r'^process/$', self.view_process_order, name='personal_process'),
        )
        return urlpatterns
